...
There are some reliability with the Census API in 2025. If things are not working, the following code provides a workaround for installing tidycensus and tigris to work correctly - https://github.com/mafichman/CPLN_675/blob/main/misc/tigris_patch.R
...
```{r}
census_api_key("6662138161177aabbe10805405e7b20b4dedb1f8", overwrite = TRUE)
```
...
```{r}
acs2020_incidents <- acs2020_incidents %>%
  mutate(n_per_sqmi = as.numeric(n / area_sqmi))
```
...
```{r}
philly_water <- area_water("PA", "Philadelphia", year = 2020)

philly_water <- philly_water %>%
  st_transform(crs = 2272)

philly_districts <- st_read("https://raw.githubusercontent.com/mafichman/cpln_0795/main/data/council_dist.geojson")
...
ggplot()+
  ...
  labs(
    title = "Incidents of 311 Complaints by Census Tracts, with Philadelphia City Council Districts Highlighted",
    caption = "Data: American Community Survey 5-year estimates, Philly 311")
```
